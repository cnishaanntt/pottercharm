...
function logRequest(requestBody, path) {
	console.log("URL : " + config.credentials.visaUrl + path);
...
function getXPayToken(resourcePath , queryParams , postBody) {
	...
	var sharedSecret = config.credentials.sharedSecret;
...
VisaAPIClient.prototype.doMutualAuthRequest = function(path, requestBody, methodType, headers, callback) {
	...
	var userId = config.credentials.userId ;
	var password = config.credentials.password;
	var keyFile = config.credentials.key;
	var certificateFile = config.credentials.cert;
	...
	request({
		uri : config.credentials.visaUrl + path,
...
VisaAPIClient.prototype.doXPayRequest = function(baseUri, resourcePath, queryParams, requestBody, methodType, headers, callback) {
	...
	headers['ex-correlation-id'] = randomstring.generate({length:12, charset: 'alphanumeric'}) + '_SC'
	request({
		uri : config.credentials.visaUrl + baseUri + resourcePath + '?' + queryParams,
		method : methodType,
		headers: headers,
		body: requestBody
	}, function(error, response, body) {
